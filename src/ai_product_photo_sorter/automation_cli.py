...
def _shopify_client_from_env(
    ...
    return ShopifyClient(
        domain,
        token,
        api_version=api_version or os.getenv("SHOPIFY_API_VERSION", API_VERSION),
    )
...
def _add_candidate_args(command: argparse.ArgumentParser) -> None:
    command.add_argument("approved_groups", type=Path)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    missing = sub.add_parser("missing-assets")
    ...
    missing_local = sub.add_parser("missing-local")
    ...
    review_queue = sub.add_parser("open-review-queue")
    review_queue.add_argument("review_manifest", type=Path)
    review_queue.add_argument("--limit", type=int, default=50)

    review_init = sub.add_parser("review-init")
    review_init.add_argument("output_root", type=Path)

    review_summary_cmd = sub.add_parser("review-summary")
    review_summary_cmd.add_argument("manifest", type=Path)

    review_apply = sub.add_parser("review-apply")
    review_apply.add_argument("manifest", type=Path)
    review_apply.add_argument("plan", type=Path)

    review_export = sub.add_parser("review-export-approved")
    review_export.add_argument("manifest", type=Path)
    review_export.add_argument("--approved-out", type=Path)

    sku_confirm = sub.add_parser("sku-confirm")
    sku_confirm.add_argument("manifest", type=Path)
    sku_confirm.add_argument("group")
    sku_confirm.add_argument("row")

    sku_clear = sub.add_parser("sku-clear")
    ...
    export.add_argument("match_manifest", type=Path)
    export.add_argument("--output", type=Path)
    export.add_argument("--profile", choices=("all", "shopify", "pim"), default="all")

    connector = sub.add_parser("prepare-connector-plan")
    connector.add_argument("export_manifest", type=Path)
    connector.add_argument("profile", type=Path)
    connector.add_argument("--output", type=Path)

    request = sub.add_parser("request-external-action")
    request.add_argument("action")
    request.add_argument("payload_json", type=Path)
    request.add_argument("output", type=Path)

    approve = sub.add_parser("approve-external-action")
    approve.add_argument("request", type=Path)
    approve.add_argument("grant", type=Path)
    approve.add_argument("--confirm", required=True)

    validate = sub.add_parser("validate-approval")
    validate.add_argument("request", type=Path)
    validate.add_argument("grant", type=Path)

    reserve = sub.add_parser("reserve-approved-action")
    reserve.add_argument("request", type=Path)
    reserve.add_argument("grant", type=Path)
    reserve.add_argument("state_dir", type=Path)

    result = sub.add_parser("record-execution-result")
    ...
    for name in ("execute-shopify-stage", "execute-shopify-publish", "execute-shopify-rollback"):
        ...
        command.add_argument("reservation", type=Path)
    ...
    reconcile = sub.add_parser("reconcile-akeneo-execution")
    reconcile.add_argument("state", type=Path)
    reconcile.add_argument("--base-url")

    odoo = sub.add_parser("execute-odoo-products")
    odoo.add_argument("request", type=Path)
    odoo.add_argument("reservation", type=Path)
    odoo.add_argument("--audit", type=Path)
    odoo.add_argument("--state", type=Path)
    odoo.add_argument("--base-url")
    odoo.add_argument("--database")

    odoo_reconcile = sub.add_parser("reconcile-odoo-execution")
    odoo_reconcile.add_argument("state", type=Path)
    odoo_reconcile.add_argument("--base-url")
    odoo_reconcile.add_argument("--database")
    ...
    storage_test = sub.add_parser("storage-test")
    storage_test.add_argument("remote")
    storage_test.add_argument("--remote-path", default="")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "missing-local":
            ...
            _emit([
                item.to_dict()
                for item in find_missing_local_images(
                    _catalog_fields(args.catalog),
        ...
        if args.command in {"propose-matches", "sku-generate"}:
            manifest, path = generate_candidates(
                args.approved_groups,
                args.catalog,
        ...
        if args.command == "review-init":
            manifest, path = initialize_review(args.output_root)
            _emit({"manifest": str(path), "summary": review_summary(manifest)})
            return 0

        if args.command == "review-summary":
            manifest, path = load_manifest(args.manifest)
            _emit({"manifest": str(path), "summary": review_summary(manifest)})
            return 0

        if args.command == "review-apply":
            manifest, path = apply_review_plan(args.manifest, args.plan)
            _emit({"manifest": str(path), "summary": review_summary(manifest)})
            return 0
        ...
        if args.command == "sku-confirm":
            manifest, path = confirm_candidate(args.manifest, args.group, args.row)
            _emit({"manifest": str(path), "summary": manifest.get("summary", {})})
            return 0

        if args.command == "sku-clear":
            manifest, path = clear_confirmation(args.manifest, args.group)
            _emit({"manifest": str(path), "summary": manifest.get("summary", {})})
            return 0
        ...
        if args.command in {"execute-shopify-stage", "execute-shopify-publish", "execute-shopify-rollback"}:
            ...
            _emit(fn(args.request, args.reservation, client, audit_path=args.audit))
            return 0
        ...
        if args.command in {"execute-akeneo-products", "execute-akeneo-rollback"}:
            ...
            if args.command == "execute-akeneo-products":
                ...
                return 0
            _emit(execute_akeneo_rollback(args.request, args.reservation, client, audit_path=args.audit))
            return 0

        if args.command == "reconcile-akeneo-execution":
            client = _akeneo_client_from_env(args.base_url)
            _emit(reconcile_akeneo_execution(args.state, client))
            return 0

        if args.command == "execute-odoo-products":
            client = _odoo_client_from_env(args.base_url, args.database)
            _emit(execute_odoo_products(
                args.request,
                args.reservation,
                client,
                audit_path=args.audit,
        ...
        if args.command == "reconcile-odoo-execution":
            client = _odoo_client_from_env(args.base_url, args.database)
            _emit(reconcile_odoo_execution(args.state, client))
            return 0
        ...
        if args.command == "storage-version":
            _emit({"version": rclone_version()})
            return 0

        if args.command == "storage-remotes":
            _emit({"remotes": list(list_remotes())})
            return 0
