...
from .akeneo_execution import AkeneoClient, STATE_MODE, _atomic_json, _normalize_base_url, _now, _read_json
from .execution_control import idempotency_key, record_execution_result

ACTION = "akeneo.rollback_products"


def _validated_inputs(request_path: str | Path, reservation_path: str | Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], Path]:
    ...
    return request, reservation, {"state_path": state_path, "state": state, "plan_id": approved_plan_id, "base_url": base_url, "targets": targets}, Path(reservation_path).expanduser().resolve()
...
def execute_akeneo_rollback(
    ...
    *,
    ...
    # Verify every target still exists before consuming the single-use approval. We do not
    # attempt to infer whether unrelated remote edits are safe to overwrite.
    for target in approved["targets"]:
        if client.get_product(target["identity"]) is None:
            raise ValueError(f"Rollback target no longer exists: {target['identity']}")
    ...
    try:
        for target in approved["targets"]:
            ...
            payload = {key: value for key, value in before.items() if key in {"enabled", "family", "categories", "groups", "parent", "values", "associations", "quantified_associations"}}
            if not payload:
        ...
        result = record_execution_result(
            ...
            status="succeeded",
            attempt=1,
            details={"connector": "akeneo", "operation": "rollback_products", "plan_id": approved["plan_id"], "base_url": approved["base_url"], "records_restored": restored, "state_path": str(approved["state_path"]), "automatic_rollback_performed": False},
            external_action_performed=True,
        )
        reservation.update({"status": "succeeded", "completed_at": _now(), "external_action_performed": True, "attempts": 1, "result_state_path": str(approved["state_path"])})
        _atomic_json(reservation_file, reservation)
        ...
        record_execution_result(
            ...
            status="failed",
            attempt=1,
            details={"connector": "akeneo", "operation": "rollback_products", "plan_id": approved["plan_id"], "base_url": approved["base_url"], "records_restored": restored, "state_path": str(approved["state_path"]), "reconciliation_required": restored > 0, "error": str(exc)},
            external_action_performed=restored > 0,
        )
        reservation.update({"status": "failed", "completed_at": _now(), "external_action_performed": restored > 0, "attempts": 1, "last_error": str(exc), "result_state_path": str(approved["state_path"])})
        _atomic_json(reservation_file, reservation)
