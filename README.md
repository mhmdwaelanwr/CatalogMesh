...
The stable `v3.2.0` release provides ready-to-run packages from the latest GitHub Release:
...
The v3.3 source UI adds compact workspace navigation for crowded tab sets and a vertically scrollable Automation Center, including mouse-wheel support. Use the header Workspace picker, `Ctrl+Tab` / `Ctrl+Shift+Tab`, or `Alt+W` to move quickly between workspaces on smaller displays.
